...
static void TDR_DoScan (void)
{
    ...
    LCD_FillRect(LCD_MakePoint(0, Y0 - 100 - 2), LCD_MakePoint(479, Y0 + 100 + 2), LCD_BLACK);
    LCD_Rectangle(LCD_MakePoint(X0, Y0 - 100), LCD_MakePoint(X0 + 400, Y0 + 100), LCD_RGB(80,80,80));
...
void TDR_Proc(void)
{
    ...
    FONT_Write(FONT_FRANBIG, LCD_WHITE, LCD_BLACK, 50, 100, "Time Domain Reflectometer mode");
